...
class SpeakerService(ListenerService):
    def __init__(self, model_name="Qwen/Qwen2-VL-2B-Instruct", device: str = "cuda:0", use_peft=True):
...
def startup():
    ...
    parser.add_argument("--model_name", type=str, default="share_models/Qwen2.5-VL-7B-Instruct_GRPO_lewis_LoRA_with_neg_SPEAKER_YoLLaVA_all_train_seed_42")
    parser.add_argument("--device", type=str, default="cuda:0")
    args, _ = parser.parse_known_args()
    print("[speaker_service] starting up, loading model...")
    speaker= SpeakerService(model_name=args.model_name, device=args.device, use_peft=True)
