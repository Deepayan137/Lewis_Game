...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            if "Qwen2-VL" in model_id:
                #**model_init_kwargs
                model = Qwen2VLForConditionalGeneration.from_pretrained(model, torch_dtype=torch.bfloat16, **model_init_kwargs)
            elif "Qwen2.5-VL" in model_id:
                model = Qwen2_5_VLForConditionalGeneration.from_pretrained(model, torch_dtype=torch.bfloat16, **model_init_kwargs)
        ...
        if processing_class is None:
            if "Qwen2-VL" in model_id or "Qwen2.5-VL" in model_id or "Aria" in model_id:
                processing_class = AutoProcessor.from_pretrained(model_id, use_fast=True, padding_side="left")
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        for key in prompt_inputs:
            if prompt_inputs[key].dtype.is_floating_point:
                prompt_inputs[key] = prompt_inputs[key].to(model_dtype)
